...
const MessageBubble = ({
  ...
  return (
    <motion.div
      ...
      className={cn("flex gap-3 mb-5", isUser ? "justify-end" : "justify-start")}
    >
      {/* AI Avatar */}
      {!isUser && (
      ...
      <div className={cn("max-w-[80%] space-y-1", isUser && "order-first")}>
        ...
        <div
          className={cn(
            "rounded-2xl px-5 py-4 relative font-heading",
            isUser
              ? "bg-primary/8 text-foreground rounded-br-md border border-primary/10"
              : "bg-card text-foreground rounded-bl-md border border-border/25 shadow-sm"
          )}
        >
          {/* Thinking time indicator */}
          {!isUser && thinkingTime && thinkingTime > 2 && (
            <div className="flex items-center gap-2 text-xs text-muted-foreground mb-3 pb-2.5 border-b border-border/20">
          ...
          {!isUser && content && !isStreaming && (
            <div className="flex items-center gap-1 mt-3 pt-2.5 border-t border-border/15">
              <button
